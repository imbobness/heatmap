...
/****** Object:  StoredProcedure [dbo].[HeatMap3GTLast48HoursLoader_Canary_RobertDees]    Script Date: 9/9/2026 7:26:41 AM ******/
SET ANSI_NULLS ON
GO
...
***************************************************************************************************************/
    SET @error_location='1'
		...
		SELECT sd.LineID,ta.DateTimeFrom,ta.DateTimeTo,sd.TagName
		FROM @TimeArray ta
		CROSS JOIN 
		(
		 SELECT ht.LineID, ht.TagName 
		 FROM maint_3GTHeatMapTags_Canary ht WITH (NOLOCK)
		 JOIN maint_Line ml ON ml.lineID = ht.LineID
	...
	FETCH NEXT FROM db_3GT_48Hr_Loader_Cursor INTO @LineID, @DateTimeFrom, @DateTimeTo, @TagName
	WHILE @@FETCH_STATUS = 0   
	BEGIN	
	
	  SET @TagValue = NULL
	  EXEC @TagValue=dbo.TagValue_Canary @DateTimeFrom,@DateTimeTo,@TagName
	  	  
      INSERT INTO heatMap3GTLast48Hours_Canary(LineID,DateTimeFrom,DateTimeTo,TagName,TagValue)
      VALUES(@LineID,@DateTimeFrom,@DateTimeTo,@TagName,@TagValue)
